...
if (!fs.existsSync(dbPath)) {
  console.log(`[INFORMACIÓN] La base de datos no existe en: ${dbPath}. Corre el descargador primero.`);
  process.exit(0);
}

const db = new ChordsDatabase(dbPath);
db.init();

try {
  const songs = db.getAllSongs();
  const dbStats = fs.statSync(dbPath);
  const dbSizeKB = (dbStats.size / 1024).toFixed(2);

  console.log('==================================================');
  console.log('          CONTENIDO DE LA BASE DE DATOS           ');
  console.log('==================================================');
  console.log(`Ruta:           ${dbPath}`);
  console.log(`Tamaño en disco: ${dbSizeKB} KB`);
